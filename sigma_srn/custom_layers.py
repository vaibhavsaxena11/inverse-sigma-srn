...
class LSTMCell(nn.Module):
    """
    A custom LSTM cell to work with functorch.vmap
    """
    ...
    def forward(self, input_val, state):
        # type: (Tensor, Tuple[Tensor, Tensor]) -> Tuple[Tensor, Tuple[Tensor, Tensor]]
        if state is not None:
            hx, cx = state
        else:
            zeros = torch.zeros(input_val.size(0), self.hidden_size, 
                                dtype=input_val.dtype, device=input_val.device)
            hx, cx = zeros, zeros
        gates = (torch.mm(input_val, self.weight_ih.t()) + self.bias_ih +
...
class Raymarcher(nn.Module):
    def __init__(self,
        ...
        hidden_size = 16
        # iSRN modification -custom LSTM cell to work with functorch.vmap
        # self.lstm = nn.LSTMCell(input_size=self.n_feature_channels,
                                # hidden_size=hidden_size)
        self.lstm = LSTMCell(input_size=self.n_feature_channels,
                                hidden_size=hidden_size)
    ...
    def forward(self,
        ...
        # iSRN modification - returning all world_coords instead
        # return world_coords[-1], depths[-1], log
        return world_coords, depths[-1], log 
