...
def load_rgb(path, sidelength=None,num_output_channels=3):
    img = imageio.imread(path)[:, :, :3]
    img = skimage.img_as_float32(img)
    if num_output_channels == 1:
        # grayscale experiment - see if grayscale has less color error in the 
        # pose optimization loss (it doesn't seem to make a difference)
        img = 0.3 * img[:,:,0] + 0.6 * img[:,:,1] + 0.1 * img[:,:,2]


    img = square_crop_img(img)

    if sidelength is not None and sidelength != img.shape[0]:
...
def load_pose(filename):
    ...
    if len(lines) == 1:
        ...
        if len(vals) == 12:
            ...
            pose[3,2] = 0.0    
            pose[3,3] = 1.0
            # this is parsing linemod data where the pose is cam_T_obj

            # this gives us obj_T_cam - we now need world_T_obj...
            pose = inv(pose)
        else:
            for i in range(16):
                pose[i // 4, i % 4] = vals[i]
