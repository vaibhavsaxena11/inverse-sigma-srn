...
# Multi-resolution training: Instead of passing only a single value, each of these command-line
# arguments take comma-separated lists. If no multi-resolution training is required, simply pass
# single values (see default values).
...
p.add_argument('--disable_sigma', action='store_true', default=False,
               help='Whether to disable nerf-like sigma prediction in the model.')
p.add_argument('--disable_makegrid', action='store_true', default=False,
               help='Whether to disable logging image grids in tensorboard.')
...
def train():
    ...
    # iSRN modification - we ran some experiments to see if grayscale was better for optimizing
    # the pose values, that is train SRNs to produce grayscale images (num_output_channels = 1)
    # instead of RGB color (num_output_channels = 3). The experiemnt was to see if grayscale
    # mitigated the color errors in the pose optimization loss that produced bad results.
    # Grayscale images doesn't really help with this...
    ...
    model = SRNsModel(num_instances=train_dataset.num_instances,
                      ...
                      disable_sigma=opt.disable_sigma,
                      disable_makegrid=opt.disable_makegrid,
                      num_output_channels=num_output_channels)
    model.train()
    model.cuda()
    ...
    for img_sidelength, max_steps, batch_size in zip(img_sidelengths, max_steps_per_sidelength,
        ...
        while True:
            for model_input, ground_truth in train_dataloader:
                ...
                if not opt.freeze_networks:
                    writer.add_scalar("pixel_generator_grad_norm", sum([p.grad.detach().norm(2)**2 for p in model.pixel_generator.parameters()])**0.5, iter)
