...
def parse_intrinsics(filepath, trgt_sidelength=None, invert_y=False):
    ...
    with open(filepath, 'r') as file:
        ...
        if len(items) == 9:
            # parsing the processed linemod data
            fx = float(items[0])
            cx = float(items[2])
            f = float(items[4])
            cy = float(items[5])
            ...
            height = 480
            width = 640
            grid_barycenter = torch.Tensor([0.0,0.0,0.0])
        else:
            f, cx, cy, _ = map(float, items)
...
def parse_intrinsics_araas(filepath, trgt_sidelength=None, invert_y=False):
    # Get camera intrinsics
    with open(filepath, 'r') as file:
        cam_K = [float(val) for val in file.readline().split(":")[1].strip()[1:-1].split(", ")]
        cam_K = np.reshape(cam_K, (3, 3))
    
    if trgt_sidelength is not None:
        width, height = 128, 128
        cam_K[0,2] = cam_K[0,2]/width * trgt_sidelength # cx
        cam_K[1,2] = cam_K[1,2]/height * trgt_sidelength # cy
        cam_K[0,0] = trgt_sidelength / width * cam_K[0,0] # fx
        # cam_K[1,1] = trgt_sidelength / height * cam_K[1,1] # fy
        cam_K[1,1] = cam_K[0,0] # TODO check how fx,fy change when scaling dims by different factors
