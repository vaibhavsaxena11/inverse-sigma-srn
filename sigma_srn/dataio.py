...
class SceneInstanceDataset():
    ...
    def __getitem__(self, idx):
        intrinsic_file_path = os.path.join(self.instance_dir, "intrinsics.txt")
        if not exists(intrinsic_file_path):
            intrinsic_file_path = os.listdir(os.path.join(self.instance_dir, "intrinsics"))[0]
            intrinsic_file_path = os.path.join(self.instance_dir, "intrinsics", intrinsic_file_path)
        intrinsics, _, _, _ = util.parse_intrinsics(intrinsic_file_path,
                                                    trgt_sidelength=self.img_sidelength)


        intrinsics = torch.Tensor(intrinsics).float()

        rgb = data_util.load_rgb(self.color_paths[idx], sidelength=self.img_sidelength, num_output_channels=self.num_output_channels)
        rgb = rgb.reshape(self.num_output_channels, -1).transpose(1, 0)
...
class SceneClassDataset(torch.utils.data.Dataset):
    ...
    def __init__(self,
        ...
        self.num_per_instance_observations = [len(obj) for obj in self.all_instances] ## num poses per instance
